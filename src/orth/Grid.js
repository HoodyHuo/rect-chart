...
class Grid {
  ...
  setup() {
    ...
    const costs = Array.from({ length: row }, () => new Array(col).fill(Grid.basicCost))
    //  number[][][]
  ...
  getWalkable(current, next) {
    ...
    return this.isValid(next) && (getWalkable ? getWalkable(this.getRealPoint(current), this.getRealPoint(next)) : true)
  }
